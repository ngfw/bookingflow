    ...
    <div class="px-4 py-6">
        @if($currentStep === 1)
            ...
            <div class="space-y-6 mobile-booking-step">
                <!-- Include Booking Flow Enhancements -->
                @include('components.booking-flow-enhancements', [
                ...
                <div class="space-y-3">
                    @foreach($services as $service)
                        <div class="mobile-service-card mobile-card bg-white rounded-xl shadow-sm overflow-hidden {{ $selectedService == $service->id ? 'selected' : '' }}">
                            <div wire:click="toggleServiceDetails({{ $service->id }})" 
            ...
            <div class="space-y-6 mobile-booking-step">
                ...
                <div class="mobile-card bg-white rounded-xl p-4 shadow-sm">
                    ...
                    <div class="flex space-x-3 mb-4">
                        <button wire:click="selectDate('{{ now()->format('Y-m-d') }}')" 
                        ...
                        <button wire:click="selectDate('{{ now()->addDay()->format('Y-m-d') }}')" 
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                @foreach($availableTimeSlots as $slot)
                                    <button wire:click="selectTimeSlot({{ json_encode($slot) }})" 
                                            class="mobile-time-slot py-3 px-4 rounded-lg border-2 text-center {{ $selectedTime == $slot['time'] ? 'border-pink-500 bg-pink-50 selected' : 'border-gray-200' }}">
        ...
        @elseif($currentStep === 4)
            ...
            <div class="text-center mobile-booking-step">
                @if($bookingConfirmed)
                    <div class="mobile-card bg-white rounded-xl p-6 shadow-sm">
...
@push('scripts')
    <script src="{{ asset('js/mobile-ux.js') }}"></script>
@endpush
